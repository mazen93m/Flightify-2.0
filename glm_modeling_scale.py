...
# ENA, CPS does not have SNOW
# BET does not work well with these models
 # FCM, CPS
alaska = ['AKN','ANC','BET','ENA','FAI','JNU']
...
# For DVT, VNY, use: family=sm.families.Gamma(link=sm.families.links.log()) <-- line 69 (poisson_training), random seed 1
...
expr = "VFR ~ Week_Day + Day + IFR + AWND + PRCP + PRCP_SQRT + TMIN + TMAX "
...
def generalizePoisson():
    ...
    #pipeline = Pipeline(steps=[('normalize', MinMaxScaler()), ('model', gen_poisson_gp1)])
    # Fit (train) the model:
    results = gen_poisson_gp1.fit(method='newton')
    #results = pipeline.fit(X_train)
    print(results.summary())
...
# =============================================================================
#     p = sns.jointplot(x=predicted_counts, 
#               y=actual_counts, 
#               data=df,
#               kind="reg",
#               color='b',
#               marginal_kws=dict(bins=100, color='r'))
#     p.fig.suptitle('{}: Predicted vs Actual Distributions and Behavior'.format(df.LOC[0]))
